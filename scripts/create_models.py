...
def train_and_register_model(model_name, run_name, X, y):
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        client.set_registered_model_alias(
            name=model_name, alias="production", version=model_info.registered_model_version
        )
        print(f"Promoted version {model_info.registered_model_version} of '{model_name}' to production.")
...
if __name__ == "__main__":
    ...
    for task_id, task_cfg in config.TASKS.items():
        ...
        try:
            ...
            train_and_register_model(task_cfg["proposer_model_name"], f"{task_id}-proposer", X, y)
            train_and_register_model(task_cfg["critic_model_name"], f"{task_id}-critic", X, y)
