...
async def _run_system_audit():
    ...
    async with httpx.AsyncClient() as client:
        health_tasks = [_check_service_health(client, name, url) for name, url in config.HEALTH_CHECK_URLS.items()]
        config_tasks = [_check_service_config(client, name, url) for name, url in config.CONFIG_URLS.items()]

        results = await asyncio.gather(*(health_tasks + config_tasks))
