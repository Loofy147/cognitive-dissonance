...
WONDERLAND_WORDS = [
    ...
    "curious", "dark", "deep", "discovers", "door", "dragon", "dream",
    "eagle", "elf", "enchanted", "enters", "explores", "finds", "follows",
    "forest", "found", "fountain", "friend", "garden", "giant", "glass",
    "gold", "green", "hidden", "imagines", "in", "inside", "island", "key",
    "king", "knight", "library", "magical", "map", "message", "mirror",
    "mountain", "mouse", "mysterious", "near", "ocean", "palace", "potion",
    "princess", "puzzle", "queen", "rabbit", "reads", "school", "secret",
    "sees", "silver", "story", "strange", "student", "studies", "teacher",
    "the", "through", "tower", "treasure", "turtle", "under", "valley",
    "village", "watches", "wise", "wizard", "wonderland", "writes",
    "draws", "dreams", "hatter", "golden", "crystal", "shiny", "diamond",
    "ruby", "emerald", "sapphire", "pearl", "locket", "white", "rabbit"
]
...
def extract_examples(prompt):
    ...
    for line in lines:
        ...
        if "->" in line:
            ...
            if any(x in line for x in ["determine", "Below", "t =", "distance =", "for:"]):
                continue
            parts = line.split("=")
            if len(parts) == 2:
                examples.append((parts[0].strip(), parts[1].strip()))
...
def solve_text(prompt):
    ...
    for inp, out in examples:
        ...
        for ow in o_words: words_from_ex.add(ow)
                    ...
                    for c, p in zip(iw, ow):
                        if c.isalpha() and p.isalpha():
                            if c in char_map and char_map[c] != p:
                                if isinstance(char_map[c], set): char_map[c].add(p)
                                else: char_map[c] = {char_map[c], p}
                            else: char_map[c] = p
    ...
    if shift_counts:
        ...
        if shift_counts[best_shift] > len(char_map) * 0.4:
             return "".join([chr((ord(c) - ord('a') - best_shift) % 26 + ord('a')) if c.isalpha() else c for c in target_cipher])
    ...
    for w in words:
        ...
        for candidate in all_candidates:
            if len(candidate) == len(w):
                matches = 0; possible = True
                for c_char, p_char in zip(w, candidate):
                    if c_char in char_map:
                        mapping = char_map[c_char]
                        if isinstance(mapping, set):
                            if p_char not in mapping: possible = False; break
                            else: matches += 1
                        else:
                            if mapping != p_char: possible = False; break
                            else: matches += 1
                if possible:
                    if matches > max_matches:
                        max_matches = matches; best_word = candidate
                    elif matches == max_matches and candidate in words_from_ex and best_word not in words_from_ex:
                        best_word = candidate
...
def solve_equations(prompt):
    ...
    for inp, out in examples:
        ...
        if len(nums) == 2:
            num_ex.append((int(nums[0]), int(nums[1]), out, re.sub(r"\d+", "", inp).strip()))
    ...
    for _ in range(5):
        ...
        for inp, out in sorted_ex:
            if inp in res:
                res = res.replace(inp, out); changed = True; break
        ...
        for inp, out in examples:
            if len(inp) == len(out):
                for c, o in zip(inp, out):
                    if c in char_map and char_map[c] != o: char_map[c] = None
                    else: char_map[c] = o
...
def solve_bits(prompt):
    ...
    for rot in range(8):
        for direction in [1, -1]:
            def rot_func(vec, n): return np.roll(vec, direction * n)
    ...
    for j in range(8):
        ...
        for i1, i2, i3 in combinations(range(8), 3):
            ...
            if consistent:
                ...
                if t_key in obs:
                    res_vec.append(obs[t_key]); found = True; break
                elif len(obs) >= 4:
                    # Logic patterns (Majority, Choice, etc.)
                    for bits in product([0, 1], repeat=8):
                        f = { k_in: bits[idx] for idx, k_in in enumerate(product([0, 1], repeat=3)) }
                        if all(f[k] == v for k, v in obs.items()):
                            res_vec.append(f[t_key]); found = True; break
                if found: break
        if found: continue
            ...
            if consistent:
                ...
                if t_key in obs:
                    ...
                    for bits in product([0, 1], repeat=4):
                        f = { k_in: bits[idx] for idx, k_in in enumerate(product([0, 1], repeat=2)) }
                        if all(f[k] == v for k, v in obs.items()):
                            res_vec.append(f[t_key]); found = True; break
                if found: break
        ...
        if not found:
            ...
            for i in range(8):
                if np.array_equal(y_col, X[:, i]):
                    res_vec.append(target_vec[i]); found = True; break
            if not found:
...
def wonderland_solver(prompt):
    ...
    if "gravitational" in p_lower: return solve_physics(prompt)
    elif "numeral system" in p_lower: return solve_numeral(prompt)
    elif "unit conversion" in p_lower: return solve_unit(prompt)
    elif "encryption rules" in p_lower: return solve_text(prompt)
    elif "applied to equations" in p_lower: return solve_equations(prompt)
    elif "bit manipulation rule" in p_lower: return solve_bits(prompt)
    return None
