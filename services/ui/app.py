...
def create_ui():
    with gr.Blocks(title="Self-Cognitive-Dissonance System") as demo:
        ...
        gr.Markdown("Select a dataset (task) to run through the private cognitive dissonance loop.")

        task_id_dropdown = gr.Dropdown(choices=list(config.TASKS.keys()), value=config.DEFAULT_TASK, label="Kaggle Dataset (Task ID)")
            ...
            with gr.Column():
                for name in task_cfg["feature_names"]:
                    if name.lower() == 'age':
                        ...
                        input_fields.append(gr.Radio(choices=[("Male", 0), ("Female", 1)], label=name, value=0))
                    else:
                        input_fields.append(gr.Number(label=name.replace("_", " ").title(), value=0))
                ...
                def run_loop(task, *args):
                    ...
                    features = {name: val for name, val in zip(task_cfg["feature_names"], args)}
                    try:
                        response = requests.post(EVALUATOR_URL, json={"features": features, "task_id": task})
                        response.raise_for_status()
                        return response.json()
                    except requests.exceptions.RequestException as e:
                        return {"error": str(e)}

                submit_btn.click(fn=run_loop, inputs=[task_id_dropdown] + input_fields, outputs=output)
